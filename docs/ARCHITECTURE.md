...
Skills are operating instructions authored through the agent resources API and stored in the database. The session system prompt receives only the name, description, version, and id of each skill enabled for the answering agent. When a request matches that catalog, pi calls the generic `Skill({ skill, args })` tool; the tool validates the skill against the enabled set, substitutes `$ARGUMENTS`, and returns the complete instructions into the conversation. Skills remain prompt-based and cannot add capabilities beyond the fixed Office toolset. There is no on-disk skill source or bundled-resource loader. MCP connection execution remains planned.
